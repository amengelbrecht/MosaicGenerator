...
class MosaicGenerator():
    def __init__(self, source_dir='./source', frames_dir='./frames'):
    ...
    def extract_frames(self, skip_frames=1, image_size=256):
        ...
        for file in os.listdir(self.source_dir):
            ...
            cap = cv2.VideoCapture(self.source_dir + '/' +filename)
            if cap != None:
            ...
            with tqdm(total=int(cap.get(cv2.CAP_PROP_FRAME_COUNT))) as pbar:
                while success:
                    ...
                    cv2.imwrite(f'{self.frames_dir}/{count}.jpg', image)
    ...
    def process_images(self):
        n, b, g, r = [], [], [], []
        directory = os.fsencode(self.frames_dir)

        for file in tqdm(os.listdir(directory)):
            filename = os.fsdecode(file)

            image = cv2.imread(self.frames_dir + '/' + filename)
    ...
    def generate_image(self, target_filename, single_use=False):
        ...
        for w in tqdm(range(width)):
            ...
            for h in range(height):
                ...
                row.append(cv2.imread(f'{self.frames_dir}/{source_df.iloc[diff.idxmin()].n}'))
